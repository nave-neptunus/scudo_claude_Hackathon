...
async def _synthesize(scenarios: list[dict]) -> list[dict]:
    ...
    response = await client.messages.create(
        ...
        messages=[{"role": "user", "content": json.dumps(scenarios, indent=2)}],
    )
...
Draft supplier notification emails for the top affected SKUs in the chosen scenario."""
    ...
    response = await client.messages.create(
        model=MODEL_BUILDER,
        max_tokens=4096,
        system=HITLGateAgent(demo_mode=True).EMAIL_SYSTEM if hasattr(HITLGateAgent, "EMAIL_SYSTEM") else _EMAIL_SYSTEM,
...
Add a "reviewer_notes" field to each email with a brief critique."""
    ...
    review_response = await client.messages.create(
        model=MODEL_PLANNER,
        max_tokens=4096,
        system="You are a senior supply chain communications reviewer. Critique and improve supplier outreach emails for factual grounding, tone, and completeness. Return ONLY valid JSON array.",
        messages=[{"role": "user", "content": review_prompt}],
    )
    review_text = next((b.text for b in review_response.content if hasattr(b, "text")), "")
