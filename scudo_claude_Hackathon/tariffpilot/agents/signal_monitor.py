...
1. run(raw_event)           — existing ReAct loop; enriches a known event via Brave Search.
2. poll_federal_register()  — discovery mode; fetches new tariff documents from the
                              ...
                              with Claude, and returns TariffEvent dicts.
"""
...
# ---------------------------------------------------------------------------
# Pydantic model for Claude extraction output (Constraint 2: validate before use)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Prompt used for Federal Register extraction (separate from Brave enrichment)
# ---------------------------------------------------------------------------
...
- rate_change_bps: integer basis-point change in tariff rate (100 bps = 1 percentage
  ...
  Example: 0% → 84% = 8400. If the document only states a final rate with no prior
  rate, set null. Return null if you cannot determine it confidently.
</rules>
...
# ---------------------------------------------------------------------------
# Existing Brave-enrichment prompt (unchanged)
# ---------------------------------------------------------------------------
...
class SignalMonitorAgent:
    ...
    # ------------------------------------------------------------------
    # MODE 1: Brave-enrichment (existing, unchanged)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # MODE 2: Federal Register polling (new)
    # ------------------------------------------------------------------
    ...
    async def poll_federal_register(
        ...
        supplied set), fetches only new documents, runs Claude extraction on each,
        ...
        # Log every API call (Constraint 7: read-only calls must still be logged)
        ...
        for doc, extraction in zip(new_docs, extractions):
            ...
            # Build a TariffEvent dict matching the SPEC §3.1 output schema
            ...
            event: dict = {
                ...
                # document_number is the dedup key — do NOT also key on URL (breaks on redirects)
    ...
    async def _extract_tariff_metadata(self, doc: dict) -> FedRegDocExtraction:
        """Call claude-sonnet-4-6 to extract hs_codes/jurisdictions/date/rate from
        ...
        if raw is None:
            raise ValueError(f"Claude returned unparseable JSON for {doc.get('document_number')}: {text[:200]}")

        # Pydantic validates field types and coerces where safe (Constraint 2)
    ...
    def _log_agent_run(self, entry: dict) -> None:
        ...
        Uses newline-delimited JSON so the file stays readable and appendable
